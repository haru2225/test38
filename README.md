...
周期セル内の完全ランダム配置から生成する別実験は
[TEST39.md](TEST39.md)を参照してください。`test39.py` と `run_test39.pbs` は
test38とは別の学習・生成チェックポイントを使います。
...
### ノイズを加えた構造が回復する過程をXYZで見る

`test38_analysis.pbs` は標準で、参照構造の各粒子に独立のガウスノイズを加えて
生成を開始します。既定のノイズ幅は学習の最大値と同じ0.75 Åです。
これはモデルが学習した範囲で構造回復を見るための設定です。

```bash
git pull --ff-only origin main
qsub test38_analysis.pbs
```

`results/intermediate/run.XXXXXXXX/generated/generation.xyz` に、ノイズを加えた初期配置
（フレーム0）から最終構造まで全ステップを保存します。既定の300ステップなら301フレームです。
時間予算で中断した場合は保存済みの有効フレームだけを出力します。
ファイルはextended XYZ形式で、周期セル・元素・CG種別・固定サイトID・生成ステップを含みます。
完了した生成では、XYZ最終フレームと同じディレクトリの`final.extxyz`について
座標・セル・元素を照合し、不一致ならエラーにします。比較時は必ず同じ
`run.XXXXXXXX/generated/` 内の2ファイルを使用してください。XYZの1フレーム目は
ノイズを加えた初期配置で、最終フレームが`final.extxyz`に対応します。
OVITO等でこのファイルを軌道として開き、再生すると構造変化を確認できます。
表示座標はセル内に折り返すので、周期境界を越える粒子が画面上で跳ぶことがあります。
ステップは生成計算の番号で、MDの物理時間ではありません。

以前と同じセル内の完全ランダム配置を試す場合は明示的に指定します。
この初期状態は学習時のノイズよりはるかに大きく構造を壊すため、
結果が崩れても学習の失敗とは判断できません。

```bash
qsub -v INITIAL_STATE=noise test38_analysis.pbs
```

従来の参照構造からの生成も同じXYZ形式で確認できます。

```bash
qsub -v INITIAL_STATE=reference test38_analysis.pbs
```

`run_test38.pbs` と `python test38.py generate` の既定初期状態は従来どおり参照構造です。
既存の生成結果をXYZへ変換するだけなら、依存パッケージのある環境で
`python export_test38_xyz.py --generated results/generated` を実行します。

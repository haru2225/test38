...
### ランダム配置から生成される過程をXYZで見る

`test38_analysis.pbs` は標準で、周期セル内に全粒子を独立・一様ランダムに配置して
生成を開始します。初期構造の位置関係は使用せず、粒子数・種類・セルを保持します。
これは周期セル内のランダム配置であり、無限空間のガウスノイズや、学習で保証された
終端分布からのサンプリングではありません。学習時は参照構造付近の変位ノイズを使用するため、
ランダム配置から正しい粘土構造に戻る保証はありません。近接・重なりが生じる場合もあります。
...
`results/intermediate/run.XXXXXXXX/generated/generation.xyz` に、初期ランダム配置
...
ランダム初期配置で、最終フレームが`final.extxyz`に対応します。
